...
首先,JS 本身是没有异步这个概念的,但是搭配宿主环境可以实现异步方式的运行方式  
虽然 v8 是单线程的,但是浏览器是多线程的,像定时器,AJAX,读写文件等,这些都不是运行在  
V8 引擎中的,而是浏览器单独的线程,所以引擎有能力做到无需等待此类任务执行完毕,只需要做到执行完毕时将回调函数放入任务队列就行,任务队列也是浏览器提供的,而非存在与 ECMA 规范中
所以我们说的异步方式,就是这些宿主环境提供的,可以后台执行的任务,通过回调的方式来运行
...
既然有此类任务,我们自然需要在任务完成时去做一些事情.而且要保证顺序
管理这些异步任务,目前有三种解决方案

### 1. 回调函数

回调就是,将一个函数作为参数,传递给某个任务,等待任务完成时  
...
返回的对象有一个 then 方法,接收两个回调函数,第一个切换成功状态时的函数,第二个切换失败状态,可以将 then 想象成注册事件回调,**每次调用 then 方法返回一个新的 Promise**,这个 Promise 会以他绑定的上一个实例的状态为自身状态,已达到链式传递
如果**这个回调返回一个新的 Promise 则会以这个新的 promise 自身**;
...
既然有成功状态 自然有失败的状态 失败状态是以 reject 来确认的,执行函数内报错也会触发 rejec,使用.catch 的回调来执行,并传入错误对象,同样可以链式调用  
并且可以捕获前面所有的错误
...
表示等待后面的异步操作完成后继续返回内部执行,就是当后面的 promise 成功时,  
会将代码下方的任务安排到微任务队列,
